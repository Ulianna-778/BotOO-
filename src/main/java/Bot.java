...
public class Bot extends TelegramLongPollingBot {
    ...
    public String getBotUsername() {
        return "@interview_developer_ogar_bot";
    }

    @Override
    public String getBotToken() {
        return "7005542747:AAHZY2Eq2qLDpp_fTQ1PkqpMEspSiGdweps";
    }
    ...
    public void isCommand(Message message) {
        ...
        if (text.equals("/start_test") && !listUsers.contains(message.getFrom().getId())) {
            ...
            System.out.println("User \"" + message.getFrom() + "\" added!");
            countBallov = 0;
    ...
    private String getPercent(int countBallov) {
        ...
        String response = message.getFrom().getFirstName() + ", Вы прошли тест и правильно выполнили " + percent + "% заданий!";
        if (percent >= 70) {
            User user = new User();
            ...
            System.out.println("Пользователь \"" + message.getFrom().getFirstName() + "\" прошёл тест на " + percent + "%.");
            countBallov = 0;

            session.save(user);
            System.out.println("User \"" + user + "\" added an Database");
            session.flush();
            session.clear();
            transaction.commit();
